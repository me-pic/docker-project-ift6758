...
class GameClient:
    ...
    def process_query(self, gameId, model_name="logistic_reg_distance_angle"):
        """
        ...
        if self.gameId == gameId:
            df = df.iloc[self.Tracker:] 
            if len(df) == 0: return None
            
        #self.game_ended = "GAME_END" in df["eventType"].values
        self.last_eventIdx = df.index[-1]


        self.gameId = gameId
        if model_name == 'logistic_reg_distance_angle':
            return df 
